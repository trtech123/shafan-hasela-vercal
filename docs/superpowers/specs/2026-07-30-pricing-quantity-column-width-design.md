...
- Give the Quantity header `w-40 min-w-[10rem]`.
- Give the matching body cell `w-40 min-w-[10rem]` with `px-2` horizontal padding.
- Give the native number input `w-full min-w-[9rem] px-2`, `dir="ltr"`, and `tabular-nums`.
...
2. The body cell cannot compress below 10rem.
3. The input cannot compress below 9rem.

...
Active phase: **Post-MVP — Phase F: Backlog batch (sites + cash register + admin modules + instructor daily invite)** ✅ **all shipped to production + browser-verified** (2026-06-17 → 2026-06-19). Worked a numbered backlog, one gated feature per commit, owner applying each migration in the SQL Editor before push. Shipped: **Task 1/2 new sites** `פודטראק`+`קפה אקסטרים` everywhere (commit `02d2561`, migration `015` widens site CHECK on orders/quotes/leads/maintenance_tasks; added to 7 frontend `SITES` arrays). **Task 7 check payment** (`cad63ee`, migration `016` `sales.payment_details` JSONB; צ'ק method + details form). **Task 4 split payments** (`62bb274`, no migration — reuses `payment_details.lines`; validates paid==total; report distributes across real methods). **Task 5 discounts** (`c02aa2a`, migration `017` `sales.discount` JSONB; הנחת עובד/נכה/כללית, %/fixed, validated; `sales.total`=final). **Task 6 link sale→order** (`314e0f5`, migration `018` `sales.order_id`+`linked_order_info`; `LinkOrderSearch` by name/phone/order#/org). **Task 13 product management** (`b4e4d5c`, migration `019` `products` table + admin-write RLS; `/products` admin page, image upload reuses `activity-images` bucket; POS not wired). **Task 12 message templates** (`cc57cf4`, migration `020` `message_templates` seeded from real texts; `/templates` admin editor — storage+editor ONLY, production sending NOT wired). **Task 8 instructor daily invitation** (`37734d9`, no migration; `InstructorDailyDialog` groups an instructor's same-day orders into one wa.me/mailto message; per-activity invite kept). Cash-register features all surface on receipt + DailySalesReport day-detail + CSV; backward-compatible (NULL columns behave as before). **Postponed, untouched:** Task 9 ticketing API, Task 10 payment gateway, Task 11 WhatsApp PDF file delivery. **Open follow-ups:** wire Task 12 templates into real sending; Schedule-side entry for the daily summary. Previous: **Phase E: Admin user management** ✅ **shipped to production + browser-verified** (2026-06-17). Also added **user deletion** (`79d55c8`, Edge Function `delete-user`, JWT+admin-gated, self-delete blocked) and a **PDF generation stabilization** (`678386c` — `document.fonts.ready` + logo decode wait, scale 2→1.5, fixed capture width, JPEG, surfaced error text). New **admin-only `ניהול משתמשים` page** (`/users`, `app/src/pages/Users.jsx`): lists all profiles (full name / email / role / created date), creates Auth users from the UI (email / password / full name / role), and edits an existing user's role inline. User creation runs through a **JWT-secured, service-role Edge Function `create-user`** (`supabase/functions/create-user/index.ts`) that verifies the caller's JWT, confirms the caller is `admin`, then uses the auto-injected `SUPABASE_SERVICE_ROLE_KEY` (server-side only — never reaches the frontend) to `auth.admin.createUser` (email auto-confirmed, `user_metadata.full_name`) and upsert the chosen role into `profiles` (overriding the trigger's default `instructor`). UI exposes only **admin / operations / cashier** — `instructor` is hidden from create/edit dropdowns but legacy instructor users still render in the list. **Self-demotion protection:** an admin's own row shows role as static text "(אתה)", not editable. Roles created/edited so far: admin, operations, cashier. No new migration needed — `003_rls.sql` already grants admin SELECT/UPDATE on `profiles`. No delete in this phase (list + create + role-edit only). Nav item `/users` (icon `UserCog`) is admin-only; non-admins are redirected from the route. Deployed with `npx supabase functions deploy create-user` (JWT on). New **admin-only `ניהול משתמשים` page** (`/users`, `app/src/pages/Users.jsx`): lists all profiles (full name / email / role / created date), creates Auth users from the UI (email / password / full name / role), and edits an existing user's role inline. User creation runs through a **JWT-secured, service-role Edge Function `create-user`** (`supabase/functions/create-user/index.ts`) that verifies the caller's JWT, confirms the caller is `admin`, then uses the auto-injected `SUPABASE_SERVICE_ROLE_KEY` (server-side only — never reaches the frontend) to `auth.admin.createUser` (email auto-confirmed, `user_metadata.full_name`) and upsert the chosen role into `profiles` (overriding the trigger's default `instructor`). UI exposes only **admin / operations / cashier** — `instructor` is hidden from create/edit dropdowns but legacy instructor users still render in the list. **Self-demotion protection:** an admin's own row shows role as static text "(אתה)", not editable. Roles created/edited so far: admin, operations, cashier. No new migration needed — `003_rls.sql` already grants admin SELECT/UPDATE on `profiles`. No delete in this phase (list + create + role-edit only). Nav item `/users` (icon `UserCog`) is admin-only; non-admins are redirected from the route. Deployed with `npx supabase functions deploy create-user` (JWT on). Previous: **Phase D: Permissions (cashier role) + internal order notes** ✅ **shipped to production** (2026-06-16, commits `624c863` + `d34b92a`, pushed to `origin/main`). New **`cashier` / קופאי** role (enum + RLS via `is_cashier()`), reworked menu access (admin=all; אחמ"ש=schedule+cashregister+orders+maintenance; קופאי=schedule+cashregister+orders), and staff-only **`orders.internal_notes`** (never customer-facing). Migrations `012`+`013`+`014` applied in Supabase; browser-verified for admin/cashier/operations. Also shipped just before: **one-time site-opening form prompt** (`f9be118`, later changed by the owner in `f3fa4cb` to show on every login for אחמ"ש only) and an owner PDF-email stabilization commit (`b791ad9`, contents not authored in this session — review before relying on it). Previous: **Phase C: Automatic order-confirmation email (Gmail SMTP)** ✅ **working in production + verified end-to-end** (2026-06-15). A `שלח במייל` button in the order-confirmation dialog generates the real combined PDF, base64-encodes it, and invokes a **JWT-secured Supabase Edge Function `send-order-doc`** that emails it as a real **PDF attachment** via **Gmail SMTP** (`npm:nodemailer`, sender **`שפן הסלע <Info.shafan@gmail.com>`**, App Password). Delivery to real customer addresses confirmed — PDF attachment arrives and opens. Recipient = `order.client_email` (validated; button disabled + Hebrew tooltip when missing/invalid). Subject `אישור הזמנה {orderNumber} - שפן הסלע`, real Hebrew body. (Resend served Stages 1–4 as the proving pipeline but is **no longer used** — it could only send to the sandbox account owner; replaced by Gmail SMTP. See the Phase C section for the full migration + denomailer→nodemailer fix.) Previous post-MVP: **Phase B: Combined order-confirmation PDF** ✅ code complete + locally browser-verified (2026-06-11). One combined customer-facing PDF reachable from any order's `FileText` action: **Section A** order confirmation (real local logo, itemized activity line — name / duration / price-per-person / participants / line total / description, total summary with `כולל מע"מ`, mobile contact `0503504478`, exact fixed order terms, signature fields) then **Section B** safety/health declaration — in one download. Delivery = **Download PDF + WhatsApp *text* helper only**; real email/WhatsApp PDF **attachment** sending is **deferred** (no provider/Edge Function exists). No schema change, no migration, no DB tracking. Commits/push deferred — awaiting user approval. Previous post-MVP: **Phase A: Leads enhancements** ✅ code complete + locally browser-verified (2026-06-07). New `customer_type` field (migration 010) with display/edit + filter chips (`סוכן`/`מפיק`/`חינוכית`/`חברה` + `ללא סיווג`); free-text search across name/phone/email/company/notes/source; `event_date` date filter (היום/השבוע/החודש/טווח תאריכים); CSV export of currently-filtered leads (UTF-8 BOM); mobile card layout (table on `md+`). Leads `site` widened to the full current business-site set `עכו`/`טבריה`/`נוף הגליל`/`שטח`/`ויה פרטה` (migration 011: added missing `נוף הגליל`; `ויה פרטה` is a real site and is kept). Both migrations applied in Supabase. Commits/push deferred — awaiting user approval. Previous post-MVP: **Israeli holidays / special dates MVP** ✅ code complete + locally browser-verified (2026-06-03). Hebcal live JSON API (Israeli mode) renders holidays/eves/national days/memorial days/fasts as small colored dots on Schedule cells with full details in the day-detail panel. No schema change, no migration, no caching table — in-memory year cache only. Admin/ops can launch "🔒 חסום יום" from a holiday entry → opens existing `BlockSlotDialog` with date + reason prefilled → creates a normal `blocked_slots` row (`source='manual'`). **No automatic blocking.** Commits/push deferred — awaiting user approval. Previous post-MVP: **Availability blocking MVP** ✅ shipped & production-verified (2026-06-03) in commits `3b9f89b`/`404bebc`/`ca39be1`/`80e5df2` — migration 009 + Schedule read/CRUD path + OrderFormDialog conflict warning with override checkbox + Orders "lock this slot" action. **Quick-win UI batch** ✅ shipped (2026-06-02) in commit `def6fcb` — mobile Orders card layout + sticky bottom order button, Orders/Activities filters, WhatsApp lite message to assigned guide, quote/proposal link on orders via existing `orders.quote_id`. **Base44 data cutover** ✅ done + browser-verified (2026-05-28); all 9 business tables hold the real Base44 data (78 rows imported); seed cleared; `profiles`/`auth` untouched. See [`docs/base44-import-run.md`](docs/base44-import-run.md). All 7 recovery phases also complete.
...
## Post-MVP — Phase D: Permissions (cashier role) + internal order notes (2026-06-16)

**Outcome:** ✅ shipped to production, pushed to `origin/main`. Two commits: **`624c863` feat: add cashier role permissions**, **`d34b92a` feat: add internal order notes**. Migrations `012`+`013`+`014` applied in Supabase. Browser-verified for admin / cashier / operations.

**Role model (current, authoritative):**
- DB enum `user_role` = `admin`, `operations`, `cashier` (NEW), `instructor` (kept for compatibility only — no active users).
- App display mapping (`AuthContext.ROLE_DISPLAY`): `admin→admin`, `operations→'אחמ"ש'`, `cashier→'קופאי'`, `instructor→'מדריך'`.
- **Business rule:** for the customer, מדריך = אחמ"ש → the active working role is **`operations`**. `instructor` is legacy/compat.
- **Menu access** (`Layout.jsx allNavItems.roles[]`):
  - **admin / מנהלים** — everything.
  - **operations / אחמ"ש** — `/schedule`, `/cashregister`, `/orders`, `/maintenance`. (No longer sees `/instructors`.)
  - **cashier / קופאי** — `/schedule`, `/cashregister`, `/orders`.
  - Redirect guard sends disallowed direct-URL hits to `/schedule` (reachable by all roles).
- **RLS** (migration `013`, additive — existing 003 policies untouched): helper `is_cashier()`; cashier granted `orders` SELECT/INSERT/UPDATE and `sales` SELECT/INSERT (no DELETE; no sales UPDATE — receipts immutable). Cashier inherits existing staff-read on `activities`/`instructors` and own-row `profiles` read. **Not** granted leads/quotes/pricing/maintenance/tasks/management. Tasks/maintenance lanes on Schedule render empty for cashier (silent RLS filter, no error).

**Internal order notes (migration `014`):**
- New `orders.internal_notes TEXT` — **staff-only, never customer-facing**. Separate from `orders.notes` (which IS customer-facing and prints in the PDF).
- Edited in `OrderFormDialog` (`🔒 הערות פנימיות` textarea, separate from `הערות (ללקוח)`); shown on `Orders` page (mobile card + desktop) and the `Schedule` order card.
- **Verified excluded** from `OrderConfirmationPDF.jsx`, `send-order-doc`, and `InstructorEmailDialog` (grep: `internal_notes` appears only in Orders.jsx, Schedule.jsx, OrderFormDialog.jsx, migration 014).

**Files (Phase D):** `app/src/components/Layout.jsx`, `app/src/lib/AuthContext.jsx`, `app/src/components/orders/OrderFormDialog.jsx`, `app/src/pages/Orders.jsx`, `app/src/pages/Schedule.jsx`, `supabase/migrations/012_add_cashier_role.sql`, `013_cashier_rls.sql`, `014_add_orders_internal_notes.sql`.

**Site-opening form prompt (shipped `f9be118`, owner-modified `f3fa4cb`):**
- `app/src/components/SiteOpeningPrompt.jsx`, mounted in `Layout.jsx`. Modal `נוהל פתיחת אתר` with two Google-Form buttons (פארק אקסטרים, אקסטרים ברקו כינרת) + `המשך למערכת`.
- **Current behavior (owner's `f3fa4cb`):** shows on **every login / system entry** for **`אחמ"ש` only** (not admin, not מדריך). No persistence (`המשך` only closes for the mounted session; reappears next login). Opening a form opens a new tab and does not dismiss. Second form URL is now a real distinct link (`forms.gle/...`).

**Pending after Phase D:**
- **Stage 3 — admin user-management screen** (not started): service-role Edge Function `create-user` (admin-gated) + `app/src/pages/Users.jsx` (email/password/full_name/role: admin|cashier|operations) + `/users` route + admin nav item. Until built, create users via Supabase Dashboard (trigger seeds `instructor`; promote role via SQL). May need a small migration if admin `profiles` UPDATE policy is missing (003 already has `profiles: admin update all`).
- **PDF/email reliability bugfix** (separate task, investigated, deferred): client-side `html2canvas` is non-deterministic across machines (fonts via Google CDN `display=swap` with no `document.fonts.ready` await; `scale:2` canvas/DPI/zoom limits; oversized base64 can exceed the Edge body limit). Symptom: broken PDF + `שליחת המייל נכשלה` on some client machines. Generic toast hides the real error. Owner commit `b791ad9` ("fix: stabilize order PDF email generation") may already address part of this — review its contents before re-investigating.
- WhatsApp **document** (file) send (needs Green API); verified Resend domain (optional); DB sent-tracking.
...
## Post-MVP — Phase A: Leads enhancements (2026-06-07)

**Outcome:** ✅ code complete + locally browser-verified (2026-06-07). Leads reorganized around **customer type** (not site, per customer correction), plus search, date filter, CSV export, and mobile usability. Two migrations applied in Supabase SQL Editor. Single frontend file touched: `app/src/pages/Leads.jsx`. Commits + push deferred — awaiting user approval.

**Migrations (both applied):**
- [`010_add_leads_customer_type.sql`](supabase/migrations/010_add_leads_customer_type.sql) — adds nullable `leads.customer_type TEXT` with NULL-safe CHECK allowing `סוכן`/`מפיק`/`חינוכית`/`חברה`. No backfill — all 36 existing leads stay valid as NULL. Net-new field (not in Base44 `Lead.jsonc`), a conscious addition per Rule B.
- [`011_widen_leads_site.sql`](supabase/migrations/011_widen_leads_site.sql) — widens `leads.site` CHECK to the full current business-site set `עכו`/`טבריה`/`נוף הגליל`/`שטח`/`ויה פרטה` + NULL. **Adds the missing `נוף הגליל`** so a lead can be set to it; **`ויה פרטה` is a real site and is kept.** Superset of the prior list → no data rewrite, no backfill. (Note: an interim revision briefly dropped `ויה פרטה`; corrected back in on 2026-06-07 after the customer confirmed it is a live site.)

**Frontend changes (`app/src/pages/Leads.jsx`):**
- **customer_type display/edit** — new "סוג לקוח" table column (header + inline display select + edit-mode select) and mobile-card badge + edit select. Inline change saves immediately via `updateCustomerType` (normalizes `"" → null`; null passes the CHECK). `startEdit`/`saveEdit` seed + normalize `customer_type`. Null renders as "ללא סיווג".
- **customer_type filter chips** — `הכל / סוכן / מפיק / חינוכית / חברה / ללא סיווג` with live counts. "הכל" = all; "ללא סיווג" = `!customer_type` (null); else exact match.
- **Free-text search** — case-insensitive `includes` across `full_name, phone, email, company, notes, source_text`. Clear button. Client-side over the loaded ≤200 rows (server-side deferred until the table grows).
- **event_date date filter** — chips `הכל / היום / השבוע / החודש / טווח תאריכים`. Lexical `YYYY-MM-DD` compare (no per-row Date parse). Sunday-start week. Null `event_date` rows appear only under "הכל". Custom range from/to inputs; empty bounds safe (no crash).
- **CSV export** — `handleExportCsv` exports the currently-filtered list. Columns: `מספר, שם מלא, טלפון, אימייל, חברה, סוג לקוח, אתר, סטטוס, תאריך אירוע, מקור, הערות`. UTF-8 BOM (Excel Hebrew), CRLF, all fields quoted + escaped. Filename `leads_YYYY-MM-DD.csv`. Button disabled when filtered list empty.
- **Mobile card layout** — desktop table now `hidden md:block`; new `md:hidden` stacked cards show name + customer_type/status badges, phone (tap-to-call)/email/company/site/event_date, notes-or-source preview, and tap-friendly הצעה/ערוך/delete actions. Edit mode renders stacked inputs + selects in the card. Reads the same `filtered` array — zero change to filter/export logic.
- **Site filter** — `SITE_OPTIONS = ["עכו","טבריה","נוף הגליל","שטח","ויה פרטה"]` (single source feeding the filter chips + all 3 site selects). All five real sites shown (`הכל` chip + the five); editing a lead to "נוף הגליל" now saves.

**All five filters combine (AND):** status • site • customer_type • date • search.

**Verification:** browser-verified each stage incrementally (display/edit, filters+search, date filter, CSV, mobile cards, site fix). `npm run build --prefix app` → EXIT 0 after every stage. Combined filtering, legacy-site behavior, and "נוף הגליל" edit all confirmed by the user.

**Out of scope (held):** email sending, order-confirmation PDF, safety questionnaires, iCredit/Rivhit, clubs/courses. Server-side lead search deferred.
